...
import getUpdateTransaction from '../../utils/instructions/updaterent';
import sendSignedTransaction from '../../utils/transactions/ sendSignedTransaction';
import { useRentProgram } from '../RentProgramProvider';


export type RentOutUpdateModalProps = {
  name: string;
  nftAddress: string;
};

const UPDATE_RENTING_ASSET_MUTATION = gql`
  mutation updateRentingAsset($tx: String!, $price: Float!, $numberOfDay: Int!, $isContinueListing: Boolean!, $nftAddress: String!) {
    updateRentingAsset(input:{
      tx: $tx,
      price: $price,
...
const RentOutUpdateModal = ({name, nftAddress}: RentOutUpdateModalProps): JSX.Element => {
  const {state, dispatch} = useAppContext();
  const {enqueueSnackbar} = useSnackbar();
  const [updateRentingAsset] = useMutation(UPDATE_RENTING_ASSET_MUTATION);
  ...
  const closeModalDeposit = () => {
    dispatch(updateStateDialogUpdateRentOut(false));
    setGear(0);
    setInputBOT(0);
    setInputDay(0);
  };
  ...
  const handleSubmitChangeRent = async (e: React.MouseEvent<any>) => {
    e.preventDefault();
    ...
    try {
      const transaction = await getUpdateTransaction(nftAddress, inputBOT, inputDay, selectedOption, rentProgram);
  ...
  return (
    <Modal
      open={state.renting.dialogUpdateRentOut}
    ...
    >
      <form onSubmit={handleSubmitChangeRent}>
        <Box
          sx={{
            color: 'text.primary',
            borderRadius: '0.6rem',
...
export default RentOutUpdateModal;
